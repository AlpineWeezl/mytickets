...
const TimeAnalyzeCard = ({ pass }) => {
	...
	useEffect(() => {
		...
		calculatedTimeProgress >= 0 &&
			calculatedTimeProgress <= 100 &&
			setTimeProgress(calculatedTimeProgress);
		if (new Date() > new Date(end)) {
			setGoneDays(
				Math.round(new Date(end) - new Date(begin) / (3600 * 24 * 1000))
			);
			setRemainingDays(0);
		}
		...
		if (new Date() >= new Date(begin) && new Date() <= new Date(end)) {
			setRemainingDays(
				Math.round((new Date(end) - new Date()) / (3600 * 24 * 1000))
			);
			setGoneDays(
				Math.round(
					(new Date(end) -
	...
	return (
		<div
			...
			className="rounded-md shadow-md p-4 bg-white"
		>
			...
			<div className="flex justify-evenly gap-3 text-left"></div>
