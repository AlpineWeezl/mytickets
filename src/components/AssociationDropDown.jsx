...
const AssociationDropDown = ({
	...
	useEffect(() => {
		axios
			.get(`${apiUrl}/associations`, {
				headers: { authorization: token },
			})
			.then((res) => {
				setAssociations(res.data.associations);
				const setInitial = async (associationId) => {
					const association = await res.data.associations.find(
						(ass) => ass._id === associationId
					);
	...
	return (
		<div className="flex flex-col pt-5">
			<button
				...
				className="flex p-4 border rounded justify-center items-center shadow-md"
			>
				{selectedAssociation
					? selectedAssociation.title
					: "Bitte wähle einen Verbund aus"}
				{!isOpen ? <ArrowDropDown /> : <ArrowDropUp />}
			</button>
			<div>
				<div
					className="absolute w-[85%] border shadow-md rounded-md bg-white divide-y max-h-[70vh] overflow-y-scroll"
					hidden={!isOpen}
				>
					{associations &&
