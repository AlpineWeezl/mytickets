...
const PassCard = ({ pass }) => {
	const { dateFormat, token } = useContext(appContext);
	...
	useEffect(() => {
		axios
			...
			.catch((err) => {
				...
				setLoading(false);
				toast.error(
					"Die genutzte Summe konnte nicht berechnet werden!"
				);
			});
		...
		>
			...
			<div className="grid grid-cols-4 grid-flow-row p-2">
				...
				<p className="text-right">
					{format(parseISO(pass.begin), dateFormat)}
				</p>
				<h4 className="text-right">Ende:</h4>
				<p className="text-right">
					{format(parseISO(pass.end), dateFormat)}
				</p>
				<h4 className="text-right">Kaufpreis:</h4>
				<p className="text-right">
					{parseFloat(pass.price).toFixed(2)} €
				</p>
