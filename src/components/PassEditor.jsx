...
const PassEditor = ({ newPass }) => {
	...
	const passHandler = (e) => {
		...
		newPass &&
			axios
				.post(`${apiUrl}/passes/user/${user._id}`, passToReq, {
					headers: { authorization: token },
				})
				.then((res) => {
					navigate(-1);
		...
		!newPass &&
			axios
				.put(`${apiUrl}/passes/${pass._id}`, passToReq, {
					headers: { authorization: token },
				})
				.then((res) => {
	...
	const deleteHandler = () => {
		axios
			.delete(`${apiUrl}/usages/passes/${pass._id}`, {
				headers: { authorization: token },
			})
			.then((res) => {
				toast.success(
					"Die zugehörigen Nutzungen wurden erfgolgreich gelöscht."
				);
			})
			.catch((err) => {
				toast.error(
					"Die zugehörigen Nutzungen konnten nicht gelöscht werden!"
				);
			});

		axios
			.delete(`${apiUrl}/passes/${pass._id}`, {
				headers: { authorization: token },
			})
			.then((res) => {
				navigate(`/passes/user/${user._id}`);
	...
	if (loading || (!newPass && !pass)) {
		return <LoaderPage />;
	}
	if (error || (!newPass && !pass)) {
		return <h3>Error...</h3>;
	}
		...
		<div className="bg-white p-2 shadow-md">
			...
			<form onSubmit={passHandler} className="flex flex-col gap-3 py-5">
				...
				<div className="flex justify-between">
					<div className="flex flex-col">
						<label htmlFor="begin" className="text-xs">
							Beginn
						</label>
						<input
							id="begin"
							...
							defaultValue={`${
								pass
									? format(parseISO(pass.begin), "yyyy-MM-dd")
									: ""
							}`}
						/>
					</div>
					<div className="flex flex-col">
						...
						<input
							...
							defaultValue={`${
								pass
									? format(parseISO(pass.end), "yyyy-MM-dd")
									: ""
							}`}
						/>
					</div>
