...
const UsageEditor = ({ newUsage, associationId }) => {
	const { dateFormat, user, verified, token } = useContext(appContext);
	...
	const usageHandler = async (e) => {
		...
		const connString = passId
			? `${apiUrl}/passes/${passId}`
			: `${apiUrl}/passes/${usage.passId}`;
		await axios
		...
		if (date.value - 1 < begin || date.value + 1 > end) {
			toast.warning(
				"Das eingegebene Datum liegt außerhalb der Gültigkeit!"
			);
			return;
		}
		if (!duration.value) {
		...
		newUsage &&
			axios
				.post(`${apiUrl}/usages/pass/${passId}`, usageToReq, {
					headers: { authorization: token },
				})
				.then((res) => {
					navigate(-1);
		...
		!newUsage &&
			axios
				.put(`${apiUrl}/usages/${usage._id}`, usageToReq, {
					headers: { authorization: token },
				})
				.then((res) => {
					navigate(-1);
	...
	const deleteHandler = () => {
		axios
			.delete(`${apiUrl}/usages/${usage._id}`, {
				headers: { authorization: token },
			})
			.then((res) => {
				navigate(-1);
	...
	if (error || (!newUsage && !usage)) {
		return <h3>Error...</h3>;
	}
	if (loading || (!newUsage && !usage)) {
		return <LoaderPage />;
	}
	...
	return (
		<div className="bg-white rounded-md shadow-md mb-14 z-0">
			<div className="flex flex-col justify-center items-center py-3 border-b-2 border-black text-center">
				...
				{user.price && (
					<h3 className="text-xl my-2">({user.username})</h3>
				)}
				{!newUsage && (
					<button
						type="button"
			...
			<div className="w-[85%] mx-auto py-3 my-3">
				...
				<form onSubmit={usageHandler} className="flex flex-col gap-5">
					...
					<div className="flex gap-3 justify-between">
						...
						<div className="flex flex-col w-1/6">
							<label htmlFor="durationUnit" className="text-xs">
								Einheit
							</label>
							<input
								id="durationUnit"
					...
					<div className="flex justify-around my-3">
						<button
							type="button"
							onClick={() => navigate(-1)}
							className="p-2 w-20 rounded shadow-md border border-red-600 bg-white text-red-600"
						>
							<ArrowBack />
						</button>
						<button
							type="submit"
							className="p-2 w-20 rounded shadow-md bg-green-700 text-white"
						>
							<Save />
						</button>
						<button
							type="reset"
							onClick={() => setSelectedCompany(!usage && null)}
							className="p-2 w-20 rounded shadow-md bg-red-600 text-white"
						>
							{" "}
							<RestartAlt />
						</button>
